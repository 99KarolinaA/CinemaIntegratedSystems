    ...
    public class OrderController : Controller
    {
            ...
            using (var workbook = new XLWorkbook())
            {
                ...
                for (int i = 1; i <= result.Count(); i++)
                {
                    ...
                    for (int p = 0; p < item.TicketInOrders.Count(); p++)
                    {
                        ...
                        worksheet.Cell(i + 1, p + 3).Value = "For the movie - "+item.TicketInOrders.ElementAt(p).OrderedTicket.Movie.Title;
                    }
                }
        ...
        public FileContentResult CreateInvoice(Guid id)
        {
            ...
            foreach (var item in result.TicketInOrders)
            {
                ...
                sb.AppendLine("Ticket for the movie "+item.OrderedTicket.Movie.Title + " with quantity of: " + item.Quantity + " and price of: " + item.OrderedTicket.Price + "$");
            }
